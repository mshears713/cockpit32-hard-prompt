...
| M2 build capture wrapper | Verified for wrapper logic with fake `idf.py` | `tests/test_idf_runner.py`, `PYTHONPATH=src pytest` | Run `uv run cockpit32 build` in Mike's ESP-IDF PowerShell against a real project. |
...
| M3 flash wrapper | Verified for wrapper/error handling with fake `idf.py` | `tests/test_idf_runner.py`, `PYTHONPATH=src pytest` | Run `uv run cockpit32 flash --port <BOX3_PORT>` with the ESP32-S3-BOX-3 connected. |
